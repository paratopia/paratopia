...
def gas_b(p,returnme=False): 
    ...
    pr = list(map(lambda i: np.dot(w[i],r1[i]),range(len(inin1))))
...
x0 = [0.41,0.98,10,0.88,0.98,10]
res_b = minimize(gas_b,x0,method="Nelder-Mead",bounds=bnds)
